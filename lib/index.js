        ...
        hostile.set('127.0.0.1', domain, (err) => {
            if (err) {
                ...
                server.on('request', (request, response) => {
                    ...
                    let exis = search(arg.pathname)
                    if (exis) {
                        localLogger.debug(arg.pathname)
                        fs.stat(resourcesPath + arg.pathname,  (err, stats) =>{
                            if (!err && stats.isFile()) {
                                console.log('200 ' + arg.pathname)
                                response.writeHead(200)
                                fs.createReadStream(resourcesPath  + arg.pathname).pipe(response)
                            }else{
                                console.log('404 ' + arg.pathname)
                        ...
                        mkdirp(resourcesPath  + d.dir, (err) => {
                            let contentType = mime.getType(arg.pathname);
                            if( !contentType ){
